package com.tallermecanico.motocicleta.domain.exception;

import com.tallermecanico.commons.domain.BaseException;

public class MotocicletaPatenteNotFoundException extends BaseException {
    private static final long serialVersionUID = 8237342515687888090L;
    public static final String DEFAULT_ERROR_CODE = "ERR_DOMAIN_NOT_FOUND";
    public static final String DEFAULT_ERROR_MESSAGE = "Domain not found.";

    public MotocicletaPatenteNotFoundException(String code, String message) {
        super(message);
        this.code = code;
    }

    public MotocicletaPatenteNotFoundException() {
        super(DEFAULT_ERROR_MESSAGE);
        this.code = DEFAULT_ERROR_CODE;
    }}
